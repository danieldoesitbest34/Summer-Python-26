...
HUNTER_COLOR = (255, 204, 229) # Pink
HEALER_COLOR = (192, 192, 192) # Gray
TANK_COLOR = (25, 51, 0)
...
class Hunter(Entity):
    """"
    Represents a hunter entity.
    can be converted into zombies
    """
    def __init__(self, x, y):
        super().__init__(x, y, HUNTER_COLOR)
        self.is_zombie = False
        self.is_hunter = True


    def kill_Zombies_(self):
        """
        Can kill zombies to protect humans
        """
        self.is_zombie = False
...
class Healer(Entity):
    ...
    def Cure_the_infected(self):
        """""
        Can cure infected humans
        """""
        self.image.fill(HUMAN_COLOR)
        self.is_human = True
...
class Tank(Entity):
    ...
    the most powerful zombie
    """
    def __init__(self, x, y, color):
        super().__init__(x, y, TANK_COLOR)
    
    def kill_Everyone(self):
        """
        Can kill humans,healers and hunters
        """
...
def main():
  ...
  # Create initial hunter
   for _ in range(40): # Start with 15 humans
      x = random.randint(0, GRID_WIDTH - 1)
      y = random.randint(0, GRID_HEIGHT - 1)
      hunter = Hunter(x, y)
      all_entities.add(Hunter)
      Hunter.add(Hunter)


  # Create initial healer
   for _ in range(40): # start
       x = random.randint(0, GRID_WIDTH - 1)
...
while running:
       ...
       for zombie in zombies:
           ...
           collided_hunters = pygame.sprite.spritecollide(hunter, zombie, False)
           collided_healers = pygames.sprite.spritecollide(healer, zombie, False)
           collided_tank = pygames.sprite.spritecollide(tank, humans, hunters, healers False)
           ...
           for hunter in collided_hunters:
               if  zombie.is_zombie: # Only kill if the Zombie is not a human 
                    zombie.remove(zombie)   # Remove from zombie group

           for healer in collided_healers:
               if human.is_zombie: # Only heal if zombie is not a human
                   zombie.remove(zombie)   # Remove from zombie group
                   human.add(human)

           for tank in collided_tank:
               if tank.is_human:  # Only kill human is not a zombie
                   human.
